...
const Dashboard = () => {
  ...
  const scanBreakouts = async () => {
    ...
    try {
      toast.info("🚀 Scanning NIFTY 50 stocks for value investing opportunities...");
      
      const params = new URLSearchParams({
        min_confidence: minConfidence.toString(),
        limit: '50'  // Scan only NIFTY 50 for focused value investing
      });
      ...
      console.log('Requesting NIFTY 50 breakout scan with params:', params.toString());
      
      // Show progress update
      toast.info("📊 Analyzing NIFTY 50 large cap stocks for value opportunities...", {
        duration: 3000
      });
      ...
      if (error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
        toast.error('🚨 Request timeout! NIFTY 50 scanning is optimized for better performance. Please try again.', {
          duration: 8000
        });
      } else if (error.response?.status === 404) {
        toast.error('API endpoint not found. Please check the backend is running.');
      } else if (error.response?.status >= 500) {
        toast.error('Server error. Backend may be overloaded. Try again in a moment.');
      } else {
        toast.error('Error scanning NIFTY 50 stocks. Please try again.');
      }
    } finally {
      setLoading(false);
    }
